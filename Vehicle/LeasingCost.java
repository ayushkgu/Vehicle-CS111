public class LeasingCost {
    ...
    private static double charger = 0.0;
    private static double dueAtSigning = 0.0; 
    private static int leaseLength = 0; 
    private static double monthlyCost = 0.0; 
    private static int mileageAllowance = 0; 
	...
	public static Vehicle createVehicle(String description){
        ...
        String [] descriptions = description.split(";");
            for (String desc : descriptions) { 
                keyVal = desc.trim().split(":");
                if (keyVal[0].equals("type"))
                    type = keyVal[1]; 
                else if (keyVal[0].equals("name"))
                    name = keyVal[1]; 
                else if (keyVal[0].equals("due"))
                    dueAtSigning = Double.parseDouble(keyVal[1]);
                else if (keyVal[0].equals("length"))
                    leaseLength = Integer.parseInt(keyVal[1]);
                else if (keyVal[0].equals("monthly"))
                    monthlyCost = Double.parseDouble(keyVal[1]);
                else if (keyVal[0].equals("mile/unit"))
                    usage = Double.parseDouble(keyVal[1]);
                else if (keyVal[0].equals("allowance"))
                    mileageAllowance = Integer.parseInt(keyVal[1]);
                else if (keyVal[0].equals("charger"))
                    charger = Double.parseDouble(keyVal[1]);
        ...
        Lease l = new Lease(dueAtSigning, leaseLength,  monthlyCost, mileageAllowance);

        Vehicle v = new Vehicle (name, f, l);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for (int i = 0; i<vehicles.length; i++){
            ...
            if (vehicles[i].getFuel().getType() == Fuel.ELECTRIC){
                ...
                CO2Emissions = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION);
                ...
                CO2Emissions = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_GASCOMBUSTION);
            ...
            vehicles[i].setCO2Emission(CO2Emissions);
            vehicles[i].setFuelCost(fuelCost);

            vehicles[i].setTotalCost(fuelCost + leaseCost + chargerCost);
            }
